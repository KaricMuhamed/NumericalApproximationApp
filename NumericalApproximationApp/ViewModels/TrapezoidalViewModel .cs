...
{
    public class TrapezoidalViewModel : INotifyPropertyChanged
    {
        ...
        public ObservableCollection<TrapezoidalIteration> Iterations { get; set; }
        public ICommand SolveCommand { get; }

        public TrapezoidalViewModel()
        {
            Iterations = new ObservableCollection<TrapezoidalIteration>();
        ...
        private void OnSolve()
        {
            try
            {
                ...
                var trapezoidal = new TrapezoidalRule
                {
                    A = a,
                    B = b,
                    N = n,
                ...
                var (result, iterations) = trapezoidal.Solve();
        ...
        private void UpdateChart(Func<double, double> function, double a, double b, int n)
        {
            ...
            // Tačke trapeza (gornji rubovi)
            var trapezPoints = new List<ObservablePoint>();
            double h = (b - a) / n;
            trapezPoints.Add(new ObservablePoint(a, 0));
            for (int i = 0; i <= n; i++)
            {
                double x = a + i * h;
                try
                {
                    double y = function(x);
                    trapezPoints.Add(new ObservablePoint(x, y));
                }
                catch { }
            }
            trapezPoints.Add(new ObservablePoint(b, 0));

            // Tačke na x osi za prikaz podjele
            ...
            ChartSeries = new ISeries[]
            {
                new LineSeries<ObservablePoint>
                {
                    ...
                    GeometrySize = 0,
                    Stroke = new SolidColorPaint(new SKColor(16, 185, 129, 120)) { StrokeThickness = 1 },
                    Name = "Trapezi",
                    LineSmoothness = 0,
